...
//	g_env.world_num = 1;
	...
	if(!mSERVER.Init(&ret))
 	...
 	if(ret == NULL) __LogMgr()->Log("服务器开启成功[%d]",mSERVER_INFO.mServerPort);
...
BOOL ReadServerInfo( void )
{
	...
	hFile = CreateFile( "CONFIG\\EXTRA_SERVER_INFO.TXT", GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL );
	...
	strcpy( mSERVER_INFO.mDB2_1_IP, tempString01 );
	if( !GetOneValueFromFile( hFile, &tempString01[0], 100 ) )
	{
		CloseHandle( hFile );
		return FALSE;
	}
	mSERVER_INFO.mDB2_1_PORT = atoi( tempString01 );
	if( !GetOneValueFromFile( hFile, &tempString01[0], 100 ) )
	{
		CloseHandle( hFile );
		return FALSE;
	}
	strcpy( mSERVER_INFO.mDB2_1_ID, tempString01 );
	if( !GetOneValueFromFile( hFile, &tempString01[0], 100 ) )
	{
		CloseHandle( hFile );
		return FALSE;
	}
	strcpy( mSERVER_INFO.mDB2_1_PASSWORD, tempString01 );
	if( !GetOneValueFromFile( hFile, &tempString01[0], 100 ) )
	{
		CloseHandle( hFile );
		return FALSE;
	}
	strcpy( mSERVER_INFO.mDB2_1_Table01, tempString01 );
...
{
	BT_InstallSehFilter();
	BT_SetAppName("SailorSky2 EXTRA BugTrap");
	BT_SetReportFilePath("d:\\BugTrap");
